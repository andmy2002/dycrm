...
class Tpl
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $class         = false !== strpos($type, '\\') ? $type : '\\think\\Tpl\\driver\\' . ucwords($type);
    ...
    public function fetch($Tpl, $vars = [], $config = [])
    {
        ...
        $Tpl = $this->parseTplFile($Tpl);
        if ($Tpl) {
            $cacheFile = $this->config['cache_path'] . $this->config['cache_prefix'] . md5($this->config['layout_name'] . $Tpl) . '.' . ltrim($this->config['cache_suffix'], '.');
    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        if ($this->config['layout_on']) {
            if (false !== strpos($content, '{__NOLAYOUT__}')) {
                ...
                // 读取布局模板
                $layoutFile = $this->parseTplFile($this->config['layout_name']);
                if ($layoutFile) {
                    // 替换布局的主体内容
    ...
    private function parseLayout(&$content)
    {
        ...
        if (preg_match($this->getRegex('layout'), $content, $matches)) {
            ...
            if (!$this->config['layout_on'] || $this->config['layout_name'] != $array['name']) {
                // 读取布局模板
                $layoutFile = $this->parseTplFile($array['name']);
                if ($layoutFile) {
    ...
    private function parseInclude(&$content)
    {
        ...
        $func  = function ($Tpl) use (&$func, &$regex, &$content) {
            if (preg_match_all($regex, $Tpl, $matches, PREG_SET_ORDER)) {
                foreach ($matches as $match) {
                    $array = $this->parseAttr($match[0]);
                    $file  = $array['file'];
                    unset($array['file']);
                    // 分析模板文件名并读取内容
                    $parseStr = $this->parseTplName($file);
    ...
    private function parseExtend(&$content)
    {
        ...
        $extend = '';
        $func   = function ($Tpl) use (&$func, &$regex, &$array, &$extend, &$blocks, &$baseBlocks) {
            if (preg_match($regex, $Tpl, $matches)) {
                if (!isset($array[$matches['name']])) {
                    $array[$matches['name']] = 1;
                    // 读取继承模板
                    $extend = $this->parseTplName($matches['name']);
                    // 递归检查继承
                    $func($extend);
                    // 取得block标签内容
                    $blocks = array_merge($blocks, $this->parseBlock($Tpl));
                    return;
                }
            } else {
                // 取得顶层模板block标签内容
                $baseBlocks = $this->parseBlock($Tpl, true);
                if (empty($extend)) {
    ...
    public function parseTagLib($tagLib, &$content, $hide = false)
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\think\\Tpl\\taglib\\' . ucwords($tagLib);
        }
        /** @var Taglib $tLib */
    ...
    public function parseVarFunction(&$varStr)
    {
        ...
        if (isset($_varFunctionList[$_key])) {
            ...
            // 取得模板禁止使用函数列表
            $Tpl_deny_funs = explode(',', $this->config['tpl_deny_func_list']);
            for ($i = 0; $i < $length; $i++) {
     ...
     * @return string
     */
    private function parseTplName($TplName)
    {
        $array    = explode(',', $TplName);
        $parseStr = '';
        foreach ($array as $TplName) {
            if (empty($TplName)) {
                continue;
            }
            if (0 === strpos($TplName, '$')) {
                //支持加载变量文件名
                $TplName = $this->get(substr($TplName, 1));
            }
            $Tpl = $this->parseTplFile($TplName);
            if ($Tpl) {
                // 获取模板文件内容
                $parseStr .= file_get_contents($Tpl);
            }
        }
        return $parseStr;
    }
     ...
     * 解析模板文件名
     * @access private
     * @param  string $Tpl 文件名
     * @return string|false
     */
    private function parseTplFile($Tpl)
    {
        if ('' == pathinfo($Tpl, PATHINFO_EXTENSION)) {
            ...
            if (0 !== strpos($Tpl, '/')) {
                $Tpl = str_replace(['/', ':'], $this->config['view_depr'], $Tpl);
            } else {
                $Tpl = str_replace(['/', ':'], $this->config['view_depr'], substr($Tpl, 1));
            }
            ...
            $Tpl = realpath($path . $Tpl . '.' . ltrim($this->config['view_suffix'], '.'));
        }

        if (is_file($Tpl)) {
            // 记录模板文件的更新时间
            $this->includeFile[$Tpl] = filemtime($Tpl);
            return $Tpl;
        } else {
