...
class Weixin {
    ...
    public function submit($user,$order,$param)
    {
        ...
        $data['nonce_str'] =  ffcms_get_rndstr();//随机字符串
        ...
        //获取付款二维码
        $data_xml = ffcms_array2xml($data);
        $res = ffcms_curl_post('https://api.mch.weixin.qq.com/pay/unifiedorder', $data_xml);
        $res = ffcms_xml2array($res);
    ...
    public function notify()
    {
        ...
        $config = config('feifeicms.pay');

        //将服务器返回的XML数据转化为数组
        $data = ffcms_xml2array($xml);
        // 保存微信服务器返回的签名sign
        $data_sign = $data['sign'];
        // sign不参与签名算法
        unset($data['sign']);
