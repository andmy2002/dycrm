        ...
        elseif ($this->_conf['gjc4'] <> '' && strstr($content, $this->_conf['gjc4'])) {
            ...
            if($res['code']>1 || empty($res['list'])){
                ...
                if($this->_conf['bofang'] ==2){
                    $search_url = $this->_conf['sousuo'] .ffcms_url('vod/search',['wd'=>$content]); //'/index.php/vod/search/wd/' . urlencode($content);
                    ...
                    foreach ($res['list'] as $k => $v) {
                        ...
                        $url = $this->_conf['sousuo'] . ffcms_url_vod_detail($v);
                        if ($this->_conf['bofang'] > 0) {
                            $url = $this->_conf['sousuo'] . ffcms_url_vod_play($v, ['sid' => 1, 'nid' => 1]);
                        }

                        if (substr($v['vod_pic'], 0, 4) == 'http' || substr($v['vod_pic'], 0, 4) == 'feifeicms:') {
                            $picUrl = ffcms_url_img($v['vod_pic']);
                        } else {
                            $picUrl = $this->_conf['sousuo'] . "/" . $v['vod_pic'];
                        }

                        $txt .= '<a href="' . $url . '">' . ($k + 1) . ',' . $v['vod_name'] . ' ' . $v['vod_remarks'] . '</a>' . "\n";
                        $data[] = array('Title' => $v['vod_name'], 'Description' => ffcms_substring(strip_tags($v["vod_content"]), 20), 'PicUrl' => $picUrl, 'Url' => $url);
                    }
                }
            }
        }
