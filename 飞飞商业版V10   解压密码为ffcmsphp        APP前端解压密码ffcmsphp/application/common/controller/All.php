...
class All extends Controller
{
    ...
    protected function load_page_cache($tpl,$type='html')
    {
        if(defined('ENTRANCE') && ENTRANCE == 'index' && $GLOBALS['config']['app']['cache_page'] ==1  && $GLOBALS['config']['app']['cache_time_page'] ) {
            $cach_name = FFCMS_MOB . '_'. $GLOBALS['config']['app']['cache_flag']. '_' .$tpl .'_'. http_build_query(ffcms_param_url());
    ...
    protected function label_feifeicms()
    {
        $feifeicms = $GLOBALS['config']['site'];
        $feifeicms['path'] = FFCMS_PATH;
        $feifeicms['path_tpl'] = FFCMS_PATH_TEMPLATE;
        $feifeicms['path_ads'] = FFCMS_PATH_ADS;
        $feifeicms['user_status'] = $GLOBALS['config']['user']['status'];
        $feifeicms['date'] = date('Y-m-d');

        $feifeicms['search_hot'] = $GLOBALS['config']['app']['search_hot'];
        $feifeicms['art_extend_class'] = $GLOBALS['config']['app']['art_extend_class'];
        $feifeicms['vod_extend_class'] = $GLOBALS['config']['app']['vod_extend_class'];
        $feifeicms['vod_extend_state'] = $GLOBALS['config']['app']['vod_extend_state'];
        $feifeicms['vod_extend_version'] = $GLOBALS['config']['app']['vod_extend_version'];
        $feifeicms['vod_extend_area'] = $GLOBALS['config']['app']['vod_extend_area'];
        $feifeicms['vod_extend_lang'] = $GLOBALS['config']['app']['vod_extend_lang'];
        $feifeicms['vod_extend_year'] = $GLOBALS['config']['app']['vod_extend_year'];
        $feifeicms['vod_extend_weekday'] = $GLOBALS['config']['app']['vod_extend_weekday'];
        $feifeicms['actor_extend_area'] = $GLOBALS['config']['app']['actor_extend_area'];

        $feifeicms['http_type'] = $GLOBALS['http_type'];
        $feifeicms['http_url'] = $GLOBALS['http_type']. ''.$_SERVER['SERVER_NAME'].($_SERVER["SERVER_PORT"]==80 ? '' : ':'.$_SERVER["SERVER_PORT"]).$_SERVER["REQUEST_URI"];
        $feifeicms['seo'] = $GLOBALS['config']['seo'];
        $feifeicms['controller_action'] = $this->_cl .'/'.$this->_ac;

        if(!empty($GLOBALS['mid'])) {
            $feifeicms['mid'] = $GLOBALS['mid'];
        }
        else{
            $feifeicms['mid'] = ffcms_get_mid($this->_cl);
        }
        if(!empty($GLOBALS['aid'])) {
            $feifeicms['aid'] = $GLOBALS['aid'];
        }
        else{
            $feifeicms['aid'] = ffcms_get_aid($this->_cl,$this->_ac);
        }
        $this->assign( ['feifeicms'=>$feifeicms] );
    }
    ...
    {
        $comment = config('feifeicms.topment');
    ...
    protected function label_type($view=0)
    {
        ...
        if($view<2) {
            ...
            if($res['code']>1){
                echo $this->error($res['msg'], ffcms_url('user/index') );
                exit;
            }
        }
        return $info;
    ...
    protected function label_actor_detail($info=[],$view=0)
    {
        ...
        $this->assign('param',$param);
        if(empty($info)) {
            $res = ffcms_label_actor_detail($param);
            if ($res['code'] > 1) {
        ...
        if($view <2) {
            ...
            if($popedom['code']>1){
                ...
                echo $this->error($popedom['msg'], ffcms_url('user/index') );
                exit;
            }
        }
    ...
    protected function label_role_detail($info=[])
    {
        ...
        $this->assign('param',$param);
        if(empty($info)) {
            $res = ffcms_label_role_detail($param);
            if ($res['code'] > 1) {
                return $this->error($res['msg']);
            }
            $info = $res['info'];
    ...
    protected function label_website_detail($info=[],$view=0)
    {
        ...
        $this->assign('param',$param);
        if(empty($info)) {
            $res = ffcms_label_website_detail($param);
            if ($res['code'] > 1) {
                return $this->error($res['msg']);
            }
            $info = $res['info'];
    ...
    protected function label_topic_index($total='')
    {
        ...
        $url = ffcms_url_topic_index(['page'=>'PAGELINK']);
        $__PAGING__ = ffcms_page_param($total,1,$param['page'],$url);
    ...
    protected function label_topic_detail($info=[])
    {
        ...
        $this->assign('param',$param);
        if(empty($info)) {
            $res = ffcms_label_topic_detail($param);
            if ($res['code'] > 1) {
                return $this->error($res['msg']);
            }
            $info = $res['info'];
    ...
    protected function label_art_detail($info=[],$view=0)
    {
        ...
        if(empty($info)) {
            $res = ffcms_label_art_detail($param);
            if ($res['code'] > 1) {
                return $this->error($res['msg']);
            }
            $info = $res['info'];
        ...
        $url = ffcms_url_art_detail($info,['page'=>'PAGELINK']);

        $__PAGING__ = ffcms_page_param($info['art_page_total'],1,$param['page'],$url);
        $this->assign('__PAGING__',$__PAGING__);
    ...
    protected function label_vod_detail($info=[],$view=0)
    {
        ...
        $this->assign('param',$param);
        if(empty($info)) {
            $res = ffcms_label_vod_detail($param);
            if ($res['code'] > 1) {
                return $this->error($res['msg']);
            }
            $info = $res['info'];
    ...
    protected function label_vod_role($info=[],$view=0)
    {
        ...
        $role = ffcms_label_vod_role(['rid'=>intval($info['vod_id'])]);
        if ($role['code'] > 1) {
    ...
    protected function label_vod_play($flag='play',$info=[],$view=0,$pe=0)
    {
        ...
        if(substr($player_info['url'],0,6) == 'upload'){
            $player_info['url'] = FFCMS_PATH . $player_info['url'];
        }
        if(substr($player_info['url_next'],0,6) == 'upload'){
            $player_info['url_next'] = FFCMS_PATH . $player_info['url_next'];
        }
        ...
        if($GLOBALS['config']['app']['encrypt']=='1'){
            $player_info['url'] = ffcms_escape($player_info['url']);
            $player_info['url_next'] = ffcms_escape($player_info['url_next']);
        }
        elseif($GLOBALS['config']['app']['encrypt']=='2'){
            $player_info['url'] = base64_encode(ffcms_escape($player_info['url']));
            $player_info['url_next'] = base64_encode(ffcms_escape($player_info['url_next']));
        }
        ...
        if( $pe==0 && $flag=='play' && ($popedom['trysee']>0 ) || ($info['vod_pwd_'.$flag]!='' && session($pwd_key)!='1') || ($info['vod_copyright']==1 && !empty($info['vod_jumpurl']) && $GLOBALS['config']['app']['copyright_status']==4) ) {
            $dy_play = ffcms_url('index/vod/'.$flag.'er',['id'=>$info['vod_id'],'sid'=>$param['sid'],'nid'=>$param['nid']]);
            $this->assign('player_data','');
            $this->assign('player_js','<div class="FeifeiPlayer" style="z-index:99999;width:100%;height:100%;margin:0px;padding:0px;"><iframe id="player_if" name="player_if" src="'.$dy_play.'" style="z-index:9;width:100%;height:100%;" border="0" marginWidth="0" frameSpacing="0" marginHeight="0" frameBorder="0" scrolling="no" allowfullscreen="allowfullscreen" mozallowfullscreen="mozallowfullscreen" msallowfullscreen="msallowfullscreen" oallowfullscreen="oallowfullscreen" webkitallowfullscreen="webkitallowfullscreen" ></iframe></div>');
        }
        else {
            $this->assign('player_data', '<script type="text/javascript">var player_data=' . json_encode($player_info) . '</script>');
            $this->assign('player_js', '<script type="text/javascript" src="' . FFCMS_PATH . 'static/js/playerconfig.js?t='.$this->_tsp.'"></script><script type="text/javascript" src="' . FFCMS_PATH . 'static/js/player.js?t='.$this->_tsp.'"></script>');
        }
        $this->label_comment();
        return $info;
    }
