...
class Actor extends Base {
    ...
    public function saveData($data)
    {
        ...
        if(empty($data['actor_blurb'])){
            $data['actor_blurb'] = ffcms_substring( strip_tags($data['actor_content']) ,100);
        }

        if($data['uptag']==1){
            $data['actor_tag'] = ffcms_get_tag($data['actor_name'], $data['actor_content']);
        }
        if($data['uptime']==1){
