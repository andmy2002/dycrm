...
class Collect extends Base {
    ...
    public function syncImages($pic_status,$pic_url,$flag='vod')
    {
        if($pic_status == 1){
            ...
            $link = FFCMS_PATH . $img_url;
            $link = str_replace('feifeicms:', $GLOBALS['config']['upload']['protocol'].':', $img_url);
    ...
    {
        if($show==1) {
            ffcms_echo('当前采集任务<strong class="green">' . $data['page']['page'] . '</strong>/<span class="green">' . $data['page']['pagecount'] . '</span>页 采集地址&nbsp;' . $data['page']['url'] . '');
        }

        $config = config('feifeicms.collect');
        $config = $config['vod'];
        ...
        $pse_name =  ffcms_txt_explain($config['namewords']);
        $pse_syn = ffcms_txt_explain($config['thesaurus']);
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif( ffcms_array_filter($filter_arr,$v['vod_name']) !==false) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['vod_id']);
                ...
                $v['vod_class'] = ffcms_txt_merge($v['vod_class'],$v['type_name']);

                $v['vod_actor'] = ffcms_format_text($v['vod_actor']);
                $v['vod_director'] = ffcms_format_text($v['vod_director']);
                $v['vod_class'] = ffcms_format_text($v['vod_class']);
                $v['vod_tag'] = ffcms_format_text($v['vod_tag']);
                ...
                if ($config['psename'] == 1) {
                    $v['vod_name'] = ffcms_rep_pse_syn($pse_name, $v['vod_name']);
                }
                if ($config['psernd'] == 1) {
                    $v['vod_content'] = ffcms_rep_pse_rnd($pse_rnd, $v['vod_content']);
                }
                if ($config['psesyn'] == 1) {
                    $v['vod_content'] = ffcms_rep_pse_syn($pse_syn, $v['vod_content']);
                }

                if(empty($v['vod_blurb'])){
                    $v['vod_blurb'] = ffcms_substring( strip_tags($v['vod_content']) ,100);
                }
                ...
                if (strpos($config['inrule'], 'f')!==false) {
                    $where['vod_actor'] = ['like', ffcms_like_arr($v['vod_actor']), 'OR'];
                }
                if (strpos($config['inrule'], 'g')!==false) {
                    $where['vod_director'] = $v['vod_director'];
                }
                if ($config['tag'] == 1) {
                    $v['vod_tag'] = ffcms_get_tag($v['vod_name'], $v['vod_content']);
                }
                ...
                if (!$info) {
                    ...
                    elseif($param['opt'] == 1){
                        ...
                        if (strpos(',' . $config['uprule'], 'n')!==false && !empty($v['vod_class']) && $v['vod_class']!=$info['vod_class']) {
                            $update['vod_class'] = ffcms_txt_merge($info['vod_class'], $v['vod_class']);
                        }
            ...
            if($show==1) {
                ffcms_echo( ($k + 1) .'、'. $v['vod_name'] . "<font color=$color>" .$des .'</font>'. $msg.'' );
            }
            else{
        ...
        if($show==1) {
            if ($param['ac'] == 'cjsel') {
                ...
                ffcms_jump($url, $GLOBALS['config']['app']['collect_timespan']);
            } else {
                if ($data['page']['page'] >= $data['page']['pagecount']) {
                    Cache::rm('collect_break_vod');
                    ffcms_echo("数据采集完成");
    ...
    public function art_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif( ffcms_array_filter($filter_arr,$v['art_name']) !==false) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['art_id']);
                ...
                if ($config['psernd'] == 1) {
                    $v['art_content'] = ffcms_rep_pse_rnd($pse_rnd, $v['art_content']);
                }
                if ($config['psesyn'] == 1) {
                    $v['art_content'] = ffcms_rep_pse_syn($pse_syn, $v['art_content']);
                }

                if(empty($v['art_blurb'])){
                    $v['art_blurb'] = ffcms_substring( strip_tags( str_replace('$$$','',$v['art_content']) ) ,100);
                }
            ...
            if($show==1) {
                ffcms_echo( ($k + 1) . $v['art_name'] . "<font color=$color>" .$des .'</font>'. $msg . '');
            }
            else{
    ...
    public function actor_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif( ffcms_array_filter($filter_arr,$v['actor_name'])!==false) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['actor_id']);
                ...
                if ($config['psernd'] == 1) {
                    $v['actor_content'] = ffcms_rep_pse_rnd($pse_rnd, $v['actor_content']);
                }
                if ($config['psesyn'] == 1) {
                    $v['actor_content'] = ffcms_rep_pse_syn($pse_syn, $v['actor_content']);
                }

                if(empty($v['actor_blurb'])){
                    $v['actor_blurb'] = ffcms_substring( strip_tags($v['actor_content']) ,100);
                }
            ...
            if($show==1) {
                ffcms_echo( ($k + 1) . $v['actor_name'] . "<font color=$color>" .$des .'</font>'. $msg . '');
            }
            else{
    ...
    public function role_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif( ffcms_array_filter($filter_arr,$v['role_name']) !==false) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['role_id']);
                ...
                if ($config['psernd'] == 1) {
                    $v['role_content'] = ffcms_rep_pse_rnd($pse_rnd, $v['role_content']);
                }
                if ($config['psesyn'] == 1) {
                    $v['role_content'] = ffcms_rep_pse_syn($pse_syn, $v['role_content']);
                }
                ...
                if (strpos($config['inrule'], 'c')!==false) {
                    $where2['vod_actor'] = ['like', ffcms_like_arr($v['role_actor']), 'OR'];
                }
                if (strpos($config['inrule'], 'd')!==false) {
                    $where2['vod_director'] = ['like', ffcms_like_arr($v['role_actor']), 'OR'];
                }
            ...
            if($show==1) {
                ffcms_echo( ($k + 1) . $v['role_name'] . "<font color=$color>" .$des .'</font>'. $msg . '');
            }
            else{
    ...
    public function website_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif( ffcms_array_filter($filter_arr,$v['website_name'])!==false) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['website_id']);
                ...
                if ($config['psernd'] == 1) {
                    $v['website_content'] = ffcms_rep_pse_rnd($pse_rnd, $v['website_content']);
                }
                if ($config['psesyn'] == 1) {
                    $v['website_content'] = ffcms_rep_pse_syn($pse_syn, $v['website_content']);
                }

                if(empty($v['website_blurb'])){
                    $v['website_blurb'] = ffcms_substring( strip_tags($v['website_content']) ,100);
                }
            ...
            if($show==1) {
                ffcms_echo( ($k + 1) . $v['website_name'] . "<font color=$color>" .$des .'</font>'. $msg . '');
            }
            else{
