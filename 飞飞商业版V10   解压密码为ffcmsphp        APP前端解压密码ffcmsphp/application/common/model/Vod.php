...
class Vod extends Base {
    ...
    public function listCacheData($lp)
    {
        ...
        if($paging=='yes') {
            ...
            if($pageurl=='vod/type' || $pageurl=='vod/show'){
                ...
                $pageurl = ffcms_url_type($type_info,$param,$flag);
            }
            else{
        ...
        if(!empty($rel)){
            $tmp = explode(',',$rel);
            if(is_numeric($rel) || ffcms_array_check_num($tmp)==true  ){
                $where['vod_id'] = ['in',$tmp];
            }
            else{
                $where['vod_rel_vod'] = ['like', ffcms_like_arr($rel),'OR'];
            }
        }
        if(!empty($level)) {
            if($level=='all'){
        ...
        if(!empty($weekday)){
            ...
            $where['vod_weekday'] = ['like', ffcms_like_arr($weekday),'OR'];
        }
        if(!empty($tv)){
        ...
        if(!empty($tag)) {
            $where['vod_tag'] = ['like',ffcms_like_arr($tag),'OR'];
        }
        if(!empty($class)) {
            $where['vod_class'] = ['like',ffcms_like_arr($class), 'OR'];
        }
        if(!empty($actor)) {
            $where['vod_actor'] = ['like', ffcms_like_arr($actor), 'OR'];
        }
        if(!empty($director)) {
            $where['vod_director'] = ['like',ffcms_like_arr($director),'OR'];
        }
        ...
        if(defined('ENTRANCE') && ENTRANCE == 'index' && $GLOBALS['config']['app']['popedom_filter'] ==1){
            $type_ids = ffcms_get_popedom_filter($GLOBALS['user']['group']['group_type']);
            if(!empty($type_ids)){
    ...
    public function infoData($where,$field='*',$cache=0)
    {
        ...
        if($GLOBALS['config']['app']['cache_core']==0 || $cache==0 || empty($info['vod_id'])) {
            ...
            if (!empty($info['vod_play_from'])) {
                $info['vod_play_list'] = ffcms_play_list($info['vod_play_from'], $info['vod_play_url'], $info['vod_play_server'], $info['vod_play_note'], 'play');
            }
            if (!empty($info['vod_down_from'])) {
                $info['vod_down_list'] = ffcms_play_list($info['vod_down_from'], $info['vod_down_url'], $info['vod_down_server'], $info['vod_down_note'], 'down');
            }
            if (!empty($info['vod_plot_name'])) {
                $info['vod_plot_list'] = ffcms_plot_list($info['vod_plot_name'], $info['vod_plot_detail']);
            }
    ...
    public function saveData($data)
    {
        ...
        if(!empty($data['vod_content'])) {
            ...
            if (!empty($match_src1)) {
                foreach ($match_src1[1] as $v1) {
                    $v2 = str_replace($GLOBALS['config']['upload']['protocol'] . ':', 'feifeicms:', $v1);
        ...
        if(empty($data['vod_blurb'])){
            $data['vod_blurb'] = ffcms_substring( strip_tags($data['vod_content']) ,100);
        }
        ...
        if($data['uptag']==1){
            $data['vod_tag'] = ffcms_get_tag($data['vod_name'], $data['vod_content']);
        }
        unset($data['uptime']);
        unset($data['uptag']);
