...
class Topic extends Base {
    ...
    public function listCacheData($lp)
    {
        ...
        if(!empty($tag)) {
            $where['topic_tag'] = ['like', ffcms_like_arr($tag),'OR'];
        }
        if(!empty($class)) {
            $where['topic_type'] = ['like', ffcms_like_arr($class),'OR'];
        }
        if($by=='rnd'){
    ...
    public function infoData($where,$field='*',$cache=0)
    {
        ...
        if($GLOBALS['config']['app']['cache_core']==0 || $cache==0 || empty($info['topic_id']) ) {
            ...
            if (!empty($info['topic_tag'])) {
                $where=[];
                $where['vod_tag'] = ['like', ffcms_like_arr($info['topic_tag']),'OR'];
                $where['vod_status'] = ['eq', 1];
                $order = 'vod_time desc';
                $field = '*';
                $res = model('Vod')->listData($where, $order, 1, 999, 0, $field);
                ...
                $where=[];
                $where['art_tag'] = ['like', ffcms_like_arr($info['topic_tag']),'OR'];
                $where['art_status'] = ['eq', 1];
                $order = 'art_time desc';
                $field = '*';
                $res = model('Art')->listData($where, $order, 1, 999, 0, $field);
    ...
    public function saveData($data)
    {
        ...
        if(empty($data['topic_blurb'])){
            $data['topic_blurb'] = ffcms_substring( strip_tags($data['topic_content']) ,100);
        }
