...
class Art extends Base {
    ...
    public function listCacheData($lp)
    {
        ...
        if(!empty($rel)){
            ...
            if(is_numeric($rel) || ffcms_array_check_num($tmp)==true ){
                ...
                $where['art_rel_art'] = ['like', ffcms_like_arr($rel),'OR'];
            }
        }
        if(!empty($letter)){
        ...
        if(!empty($tag)) {
            $where['art_tag'] = ['like', ffcms_like_arr($tag),'OR'];
        }
        if(!empty($class)) {
            $where['art_class'] = ['like',ffcms_like_arr($class),'OR'];
        }
    ...
    public function infoData($where,$field='*',$cache=0)
    {
        ...
        if($GLOBALS['config']['app']['cache_core']==0 || $cache==0 || empty($info['art_id'])) {
            ...
            if (!empty($info['art_content'])) {
                $info['art_page_list'] = ffcms_art_list($info['art_title'], $info['art_note'], $info['art_content']);
    ...
    public function saveData($data)
    {
        ...
        if(empty($data['art_blurb'])){
            $data['art_blurb'] = ffcms_substring( str_replace('$$$','', strip_tags($data['art_content'])),100);
        }
        ...
        if($data['uptag']==1){
            $data['art_tag'] = ffcms_get_tag($data['art_name'], $data['art_content']);
        }
        unset($data['uptime']);
        unset($data['uptag']);
