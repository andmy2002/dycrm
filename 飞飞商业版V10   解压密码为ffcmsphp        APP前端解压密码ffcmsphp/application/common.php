...
// 应用公共文件
function ffcms_return($msg,$code=1,$data=''){
    if(is_array($msg)){
...
function ffcms_run_statistics()
{
    $t2 = microtime(true) - FFCMS_START_TIME;
    $size = memory_get_usage();
    $memory = ffcms_format_size($size);
    unset($unit);
    return 'Processed in: '.round($t2,4).' second(s),&nbsp;' . $memory . ' Mem On.';
}

function ffcms_format_size($s=0)
{
	if($s==0){ return '0 kb'; }
...
function ffcms_read_file($f)
{
    return @file_get_contents($f);
}

function ffcms_write_file($f,$c='')
{
    $dir = dirname($f);
    if(!is_dir($dir)){
        ffcms_mkdirss($dir);
...
function ffcms_mkdirss($path,$mode=0777)
{
    if (!is_dir(dirname($path))){
        ffcms_mkdirss(dirname($path));
    }
    if(!file_exists($path)){
...
function ffcms_rmdirs($dirname, $withself = true)
{
    if (!is_dir($dirname))
        return false;
...
function ffcms_copydirs($source, $dest)
{
    if (!is_dir($dest))
    {
        mkdir($dest, 0755);
...
function ffcms_arr2file($f,$arr='')
{
    ...
    $con = "<?php\nreturn $con;";
    ffcms_write_file($f, $con);
}

function ffcms_replace_text($txt,$type=1)
{
    if($type==1){
...
function ffcms_compress_html($s){
    $s = str_replace(array("\r\n","\n","\t"), array('','','') , $s);
    $pattern = array (
        "/> *([^ ]*) *</",
        "/[\s]+/",
...
function ffcms_build_regx($regstr,$regopt)
{
    return '/'.str_replace('/','\/',$regstr).'/'.$regopt;
}

function ffcms_reg_replace($str,$rule,$value)
{
    $res='';
    $rule = ffcms_build_regx($rule,"is");
    if (!empty($str)){
...
function ffcms_reg_match($str,$rule)
{
    $res='';
    $rule = ffcms_build_regx($rule,"is");
...
function ffcms_redirect($url,$obj='')
{
    echo '<script>'.$obj.'location.href="' .$url .'";</script>';
    exit;
}

function ffcms_alert($str)
{
    echo '<script>alert("' .$str. '\t\t");history.go(-1);</script>';
}

function ffcms_alert_url($str,$url)
{
    echo '<script>alert("' .$str. '\t\t");location.href="' .$url .'";</script>';
}

function ffcms_jump($url,$sec=0)
{
...
function ffcms_echo($str)
{
    echo $str.'<br>';
    ob_flush();flush();
}

function ffcms_day($t,$f='',$c='#FF0000')
{
    if(empty($t)) { return ''; }
    if(is_numeric($t)){
...
function ffcms_friend_date($time)
{
    if (!$time)
        return false;
    $fdate = '';
...
function ffcms_get_time_span($sn)
{
    $lastTime = session($sn);
...
function ffcms_get_rndstr($length=32,$f='')
{
...
function ffcms_convert_encoding($str,$nfate,$ofate){
...
function ffcms_get_refer()
{
    return trim(urldecode($_SERVER["HTTP_REFERER"]));
}

function ffcms_send_mail($to, $title, $body,$conf=[]) {
    $config = config('feifeicms.email');
    if(!empty($conf)){
        $config = $conf;
    }
...
function ffcms_check_back_link($url)
{
    $res=[];
    $res['code'] = 0;
    $res['msg'] = '参数错误';
...
function ffcms_list_to_tree($list, $pk='id',$pid = 'pid',$child = 'child',$root=0)
{
    $tree = array();
    if(is_array($list)) {
        $refer = array();
...
function ffcms_str_correct($str,$from,$to)
{
    return str_replace($from,$to,$str);
}

function ffcms_buildregx($regstr,$regopt)
{
    return '/'.str_replace('/','\/',$regstr).'/'.$regopt;
}

function ffcms_em_replace($s)
{
    return preg_replace("/\[em:(\d{1,})?\]/","<img src=\"". FFCMS_PATH ."static/images/face/$1.gif\" border=0/>",$s);
}

function ffcms_page_param($record_total, $page_size, $page_current, $page_url,$page_half=5)
{
    $page_param = array();
    $page_num = array();
    ...
    $page_param['page_sp'] = FFCMS_PAGE_SP;
...
// CurlPOST数据提交-----------------------------------------
function ffcms_curl_post($url,$data,$heads=array(),$cookie='')
{
    $ch = @curl_init();
...
// CurlPOST数据提交-----------------------------------------
function ffcms_curl_get($url,$heads=array(),$cookie='')
{
    $ch = @curl_init();
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36');
...
function ffcms_substring($str, $lenth, $start=0)
{
    $len = strlen($str);
    $r = array();
    $n = 0;
...
function ffcms_array2xml($arr,$level=1)
{
    $s = $level == 1 ? "<xml>" : '';
...
function ffcms_array_rekey($arr,$key)
{
    $list = [];
    foreach($arr as $k=>$v){
        $list[$v[$key]] = $v;
...
function ffcms_array_filter($arr,$str)
{
    if(!is_array($arr)){
        $arr = explode(',',$arr);
    }
...
function ffcms_parse_sql($sql='',$limit=0,$prefix=[])
{
    // 被替换的前缀
    $from = '';
    // 要替换的前缀
...
function ffcms_rep_pse_rnd($psearr,$txt,$id=0)
{
    if(empty($txt)){ $txt=""; }
    if(empty($id)){
        $id = crc32($txt);
    ...
    if ($j>0){
        $res= ffcms_substring($txt,$j-1) . $psearr[$id % $i] . ffcms_substring($txt,mb_strlen($txt)-$j,$j);
    }
    else{
        $res= $psearr[$id % 1]. $txt;
    }
...
function ffcms_txt_explain($txt)
{
    $txtarr = explode('#',$txt);
    $data=[];
...
function ffcms_rep_pse_syn($psearr,$txt)
{
    if(empty($txt)){ $txt=""; }
...
function ffcms_get_tag($title,$content){
    $url = base64_decode('aHR0cDovL2FwaS5mZWlmZWljbXMudG9wLw==').'/keyword/index/txt/'.rawurlencode($title).rawurlencode(ffcms_substring(strip_tags($content),200));
    $data = ffcms_curl_get($url);
...
function ffcms_get_uniqid_code($code_prefix='')
{
...
function ffcms_escape($string, $in_encoding = 'UTF-8',$out_encoding = 'UCS-2') {
    $return = '';
...
function ffcms_get_user_status_text($data)
{
    $arr = [0=>'禁用',1=>'启用'];
    return $arr[$data];
}
function ffcms_get_user_flag_text($data)
{
    $arr = [0=>'计点',1=>'计时',2=>'ip段'];
    return $arr[$data];
}

function ffcms_get_ulog_type_text($data)
{
    $arr = [1=>'浏览',2=>'收藏',3=>'想看',4=>'点播',5=>'下载'];
    return $arr[$data];
}

function ffcms_get_plog_type_text($data)
{
    $arr = [1=>'积分充值',2=>'注册推广',3=>'访问推广',4=>'三级分销',7=>'积分升级',8=>'积分消费',9=>'积分提现'];
    return $arr[$data];
}

function ffcms_get_card_sale_status_text($data)
{
    $arr = [0=>'未出售',1=>'已出售'];
    return $arr[$data];
}

function ffcms_get_card_use_status_text($data)
{
    $arr = [0=>'未使用',1=>'已使用'];
    return $arr[$data];
}

function ffcms_get_order_status_text($data)
{
    $arr = [0=>'未支付',1=>'已支付'];
    return $arr[$data];
}
function ffcms_get_user_portrait($user_id)
{
    $res = FFCMS_PATH . 'static/images/touxiang.png';
    if(!empty($user_id)){
...
function ffcms_filter_html($str)
{
    return strip_tags($str);
}

function ffcms_format_text($str)
{
    return str_replace(array('/','，','|','、',' ',',,,'),',',$str);
}
function ffcms_format_count($str)
{
    $arr = explode(',',$str);
    return count($arr);
}

function ffcms_txt_merge($txt,$str)
{
    if(empty($str)){
        return $txt;
    }
    ...
    $txt = ffcms_format_text($txt);
    $str = ffcms_format_text($str);
    $arr1 = explode(',',$txt);
    $arr2 = explode(',',$str);
...
function ffcms_array_check_num($arr)
{
    if(!is_array($arr)){
        return false;
    }
...
function ffcms_art_list($art_title,$art_note,$art_content)
{
    $art_title_list = [];
    $art_note_list = [];
    $art_content_list = [];
...
function ffcms_plot_list($vod_plot_name,$vod_plot_detail)
{
    $vod_plot_name_list = [];
...
function ffcms_play_list($vod_play_from,$vod_play_url,$vod_play_server,$vod_play_note,$flag='play')
{
    $vod_play_from_list = [];
    $vod_play_url_list = [];
    ...
    foreach($vod_play_from_list as $k=>$v){
        ...
        $urls = ffcms_play_list_one($vod_play_url_list[$k],$v);
    ...
    if( (ENTRANCE!='admin' && FFCMS_PLAYER_SORT=='1') ||  $GLOBALS['ismake']=='1' ){
...
function ffcms_play_list_one($url_one, $from_one, $server_one=''){
    $url_list = array();
...
function ffcms_filter_words($str)
{
    $config = config('feifeicms.app');
...
function ffcms_long2ip($ip){
    $ip = long2ip($ip);
    $reg2 = '~(\d+)\.(\d+)\.(\d+)\.(\d+)~';
    return preg_replace($reg2, "$1.$2.*.*", $ip);
}
function ffcms_default($s,$def='')
{
    if(empty($s)){
        return $def;
    }
...
function ffcms_multisort($arr,$col_sort,$sort_order,$col_status='',$status_val='')
{
    $sort=[];
    foreach($arr as $k=>$v){
        $sort[] = $v[$col_sort];
...
function ffcms_get_body($text,$start,$end)
{
...
function ffcms_find_array($text,$start,$end)
{
    $start=stripslashes($start);
    $end=stripslashes($end);
    if(empty($text)){ return false; }
    ...
    $labelRule = $start."(.*?)".$end;
    $labelRule = ffcms_buildregx($labelRule,"is");
...
function ffcms_tpl_fetch($model,$tpl,$def='')
{
    return $model . '/' . ( empty($tpl) ? $def  : str_replace('.html','',$tpl) );
}

function ffcms_get_order($order,$param)
{
...
function ffcms_url_img($url)
{
    if(substr($url,0,4) == 'feifeicms:'){
        $protocol = $GLOBALS['config']['upload']['protocol'];
        if(empty($protocol)){
            $protocol = 'http';
        }
        $url = str_replace('feifeicms:', $protocol.':',$url);
    }
...
function ffcms_url_content_img($content)
{
    $protocol = $GLOBALS['config']['upload']['protocol'];
    if(empty($protocol)){
        $protocol = 'http';
    }
    return str_replace('feifeicms:',$protocol.':',$content);
}

function ffcms_url($model,$param=[],$info=[])
{
    ...
    if(!empty($path)) {
        ...
        elseif($config['rewrite']['status']==0 && strpos($url,'index.php')===false){
            if(FFCMS_PATH !='/'){
                $url = str_replace(FFCMS_PATH,'/',$url);
            }
            $url = FFCMS_PATH. 'index.php' . $url;
        }
...
function ffcms_url_page($url,$num)
{
    $url = str_replace(FFCMS_PAGE_SP.'PAGELINK',($num>1 ? FFCMS_PAGE_SP.$num : ''),$url);
    $url = str_replace('PAGELINK',$num,$url);
    return $url;
}

function ffcms_url_create($str,$type='actor',$flag='vod',$ac='search',$sp='&nbsp;')
{
    if(!$str){
        return '未知';
    }
    ...
    $arr = explode(',',$str);
    foreach($arr as $k=>$v){
        $res[$k] = '<a href="'.ffcms_url($flag.'/'.$ac,[$type=>$v]).'" target="_blank">'.$v.'</a>'.$sp;
    }
    return implode('',$res);
}

function ffcms_url_search($param=[],$flag='vod')
{
    return ffcms_url($flag.'/search',$param);
}

function ffcms_url_type($info,$param=[],$flag='type')
{
    $tab = 'vod';
    if($info['type_mid'] == 1){
    ...
    return ffcms_url($tab.'/'.$flag,$param,$info);
}

function ffcms_url_topic_index($param=[])
{
    return ffcms_url('topic/index',['page'=>$param['page']]);
}

function ffcms_url_topic_detail($info)
{
    return ffcms_url('topic/detail',[],$info);
}

function ffcms_url_role_index($param=[])
{
    return ffcms_url('role/index',['page'=>$param['page']]);
}

function ffcms_url_role_detail($info)
{
    return ffcms_url('role/detail',[],$info);
}
function ffcms_url_actor_index($param=[])
{
    return ffcms_url('actor/index',['page'=>$param['page']]);
}
function ffcms_url_actor_detail($info)
{
    return ffcms_url('actor/detail',[],$info);
}
function ffcms_url_actor_search($param)
{
    return ffcms_url('actor/search',$param);
}
function ffcms_url_plot_index($param=[])
{
    return ffcms_url('plot/index',['page'=>$param['page']]);
}
function ffcms_url_plot_detail($info,$param=[])
{
    return ffcms_url('plot/detail',['page'=>$param['page']],$info);
}
function ffcms_url_vod_plot($info,$param=[])
{
    return ffcms_url('vod/plot',$param,$info);
}
function ffcms_url_website_index($param=[])
{
    return ffcms_url('website/index',['page'=>$param['page']]);
}
function ffcms_url_website_detail($info)
{
    return ffcms_url('website/detail',[],$info);
}
function ffcms_url_website_search($param)
{
    return ffcms_url('website/search',$param);
}
function ffcms_url_art_index($param=[])
{
    return ffcms_url('art/index',['page'=>$param['page']]);
}
function ffcms_url_art_detail($info,$param=[])
{
    return ffcms_url('art/detail',['page'=>$param['page']],$info);
}
function ffcms_url_art_search($param)
{
    return ffcms_url('art/search',$param);
}
function ffcms_url_vod_index($param=[])
{
    return ffcms_url('vod/index',['page'=>$param['page']]);
}
function ffcms_url_vod_detail($info)
{
    return ffcms_url('vod/detail',[],$info);
}
function ffcms_url_vod_search($param)
{
    return ffcms_url('vod/search',$param);
}
function ffcms_url_vod_play($info,$param=[])
{
    if($param=='first'){
    ...
    return ffcms_url('vod/play',['sid'=>$param['sid'],'nid'=>$param['nid']],$info);
}

function ffcms_url_vod_down($info,$param=[])
{
    if($param=='first'){
    ...
    return ffcms_url('vod/down',['sid'=>$param['sid'],'nid'=>$param['nid']],$info);
}



function ffcms_label_website_detail($param)
{
    $where = [];
...
function ffcms_label_actor_detail($param)
{
    $where = [];
    if(is_numeric($param['id'])){
...
function ffcms_label_role_detail($param)
{
    $where = [];
    if(is_numeric($param['id'])){
...
function ffcms_label_topic_detail($param)
{
    $where = [];
    if(is_numeric($param['id'])){
...
function ffcms_label_art_detail($param)
{
    $where = [];
    if(is_numeric($param['id'])){
...
function ffcms_label_vod_detail($param)
{
    $where = [];
    if(is_numeric($param['id'])){
...
function ffcms_label_vod_role($param)
{
    $where = [];
...
function ffcms_label_type($param)
{
...
function ffcms_data_count($tid=0,$range='all',$flag='vod')
{
...
function ffcms_get_popedom_filter($group_type,$type_list=[])
{
    if(empty($type_list)){
        $type_list = model('Type')->getCache('type_list');
    }
...
function reset_html_filename($htmlfile)
{
    ...
    if(FFCMS_PATH !='/'){
        $htmlfile = str_replace('.'.FFCMS_PATH, './', $htmlfile);
    }
