...
class Urlsend extends Base
{
    ...
    public function data()
    {
        ...
        ffcms_echo('共'.$res['total'].'条数据等待推送，分'.$res['pagecount'].'页推送，当前第'.$res['page'].'页');
        ...
        foreach($res['list'] as $k=>$v){
            ...
            ffcms_echo($v[$col.'_id'] . '、'. $v[$col . '_name'] . '&nbsp;<a href="'.$urls[$v[$col.'_id']].'">'.$urls[$v[$col.'_id']].'</a>');
        }
    ...
    public function baidu_push()
    {
        ...
        if (!empty($res['urls'])) {
            ...
            $r = ffcms_curl_post($api, $data, $head);
            ...
            elseif($json['error']){
                ffcms_echo('发生错误：'. $json['message'] );
                return;
            }
            Cache::set($this->_cache_name, $this->_lastid);
            ffcms_echo('推送成功'.$json['success'].'条；当天剩余可推'.$json['remain'].'条。');
        }
        ...
        if ($res['page'] >= $res['pagecount']) {
            ...
            ffcms_echo('数据推送完毕');
            if(ENTRANCE=='admin') {
                ffcms_jump(url('urlsend/index'), 3);
            }
        }
        else {
    ...
    public function baidu_bear()
    {
        ...
        if(!empty($res['urls'])){
            ...
            elseif($json['success_realtime'] ==0 && $json['remain_realtime']>0){
                ...
                $json = json_decode($r,true);
                if(!$json){
                    ffcms_echo('请求失败，请重试2');
                    return;
                }
                elseif($json['error']){
                    ffcms_echo('发生错误2：'. $json['message'] );
                    return;
                }
            }
            ...
            Cache::set($this->_cache_name, $this->_lastid);
            if($type=='realtime'){
                ffcms_echo('熊掌号实时推送'.$json['success_realtime'].'条；熊掌号实时剩余可推送'.$json['remain_realtime'].'条.');
            }
            else{
                ffcms_echo('熊掌号历史推送'.$json['success_batch'].'条；熊掌号历史剩余可推送'.$json['remain_batch'].'条；');
            }
        }
