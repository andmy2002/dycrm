...
class Make extends Base
{
    ...
    protected function buildHtml($htmlfile='',$htmlpath='',$TplFile='') {
        if(empty($htmlfile) || empty($htmlpath) || empty($TplFile)){
            return false;
        }
        $content    =   $this->label_fetch($TplFile);
    ...
    public function opt()
    {
        ...
        //自定义页面
        $label_list = [];
        $path = FFCMS_ROOT_TEMPLATE .'label';
        if(is_dir($path)){
            $farr = glob($path.'/*');
            foreach($farr as $f){
                if(is_file($f)){
    ...
    public function index()
    {
        ...
        $GLOBALS['aid'] = ffcms_get_aid('index');
        $this->label_feifeicms();
        $link = 'index.html';
        $this->buildHtml($link,'./', 'index/index');
        $this->echoLink($link,'/'.$link);
        if(ENTRANCE=='admin'){
            ffcms_jump( url('make/opt'),3 );
        }
        exit;
    }
    ...
    public function rss()
    {
        ...
        for($i=1;$i<=$this->_param['ps'];$i++){
            ...
            $link = ffcms_url('rss/'.$this->_param['ac2'],$par);
    ...
    public function type()
    {
        if($this->_param['tab'] =='art'){
            ...
            $GLOBALS['mid'] = 2;
            $GLOBALS['aid'] = ffcms_get_aid('art','type');
        }
        else{
            ...
            $GLOBALS['mid'] = 1;
            $GLOBALS['aid'] = ffcms_get_aid('vod','type');
        }

        if($GLOBALS['config']['view'][$this->_param['tab'].'_type'] <2){
            ffcms_echo('浏览模式非静态，无法生成');
            exit;
        }
        ...
        if ($num>=count($ids)){
            if(empty($this->_param['jump'])){
                ...
                if(ENTRANCE=='admin') {
                    ffcms_jump(url('make/index', ['jump' => 1]), 3);
                }
                exit;
            }
        }
        ...
        if(empty($data_count)){
            ...
            if($this->_param['tab'] =='art') {
                ...
                $html = ffcms_read_file(FFCMS_ROOT_TEMPLATE . 'art/'.$type_info['type_tpl']);
                $labelRule = '{feifeicms:art(.*?)num="(.*?)"(.*?)paging="yes"([\s\S]*?)}([\s\S]*?){/feifeicms:art}';
            }
            else{
                $where['vod_status'] = ['eq', 1];
                $data_count = model('Vod')->countData($where);
                $html = ffcms_read_file(FFCMS_ROOT_TEMPLATE . 'vod/'.$type_info['type_tpl']);
                $labelRule = '{feifeicms:vod(.*?)num="(.*?)"(.*?)paging="yes"([\s\S]*?)}([\s\S]*?){/feifeicms:vod}';
            }

            $labelRule = ffcms_buildregx($labelRule,"");
        ...
        for($i=$start;$i<=$page_count;$i++){
            ...
            $link = ffcms_url_type($type_info,['id'=>$id,'page'=>$i]);

            $this->buildHtml($link,'./', ffcms_tpl_fetch($this->_param['tab'],$type_info['type_tpl'],'type') );
        ...
        if(ENTRANCE=='api'){
            if ($num+1>=count($ids)) {
                ffcms_echo('定时任务完毕，本次每个分类生成' . $GLOBALS['config']['app']['makesize'] . '个列表页面，避免网站卡死！');
                die;
            }
            else{
                $this->_param['start'] = 1;
    ...
    public function topic_index()
    {
        ...
        $GLOBALS['mid'] = 3;
        $GLOBALS['aid'] = ffcms_get_aid('topic');
        ...
        if(empty($data_count)){
            ...
            $html = ffcms_read_file(FFCMS_ROOT_TEMPLATE . 'topic/index.html');
            $labelRule = '{feifeicms:topic(.*?)num="(.*?)"(.*?)paging="yes"([\s\S]*?)}([\s\S]*?){/feifeicms:topic}';
        ...
        for($i=$start;$i<=$page_count;$i++){
            ...
            $link = ffcms_url_topic_index(['page'=>$i]);
    ...
    public function topic_info()
    {
        ...
        $GLOBALS['mid'] = 3;
        $GLOBALS['aid'] = ffcms_get_aid('topic','detail');
        ...
        foreach($ids as $a){
            ...
            if($res['code'] == 1) {
                ...
                $link = ffcms_url_topic_detail($topic_info);
                $this->buildHtml($link,'./', ffcms_tpl_fetch('topic',$topic_info['topic_tpl'],'detail') );
        ...
        if($this->_param['ref'] ==1 && !empty($_SERVER["HTTP_REFERER"])){
            if(ENTRANCE=='admin'){
                ffcms_jump($_SERVER["HTTP_REFERER"],2);
            }
            die;
        }
    ...
    public function info()
    {
        ...
        if(!empty($type_ids)){
            ...
            if ($num>=count($type_ids)){
                ...
                if(empty($this->_param['jump'])){
                    ...
                    if(ENTRANCE=='admin'){
                        ffcms_jump( url('make/make',['jump'=>1,'ac'=>'type','tab'=>$this->_param['tab'], $this->_param['tab'].'type'=> join(',',$type_ids) ,'ac2'=>'day']) ,3);
                    }
                    exit;
                }
            }
        ...
        foreach($res['list'] as $k=>$v){
            ...
            if(!empty($v['art_id'])) {
                ...
                $GLOBALS['mid'] = 2;
                $GLOBALS['aid'] = ffcms_get_aid('art','detail');

                $this->label_feifeicms();
                $_REQUEST['id'] = $v['art_id'];
                echo ffcms_substring($v['art_name'],100) .'&nbsp;';

                if(!empty($v['art_content'])) {
                    $art_page_list = ffcms_art_list($v['art_title'], $v['art_note'], $v['art_content']);
                    $art_page_total = count($art_page_list);
                }

                for($i=1;$i<=$art_page_total;$i++){
                    $v['art_page_list'] = ffcms_art_list($v['art_title'], $v['art_note'], $v['art_content']);
                    $v['art_page_total'] = count($v['art_page_list']);
                    $_REQUEST['page'] = $i;

                    $info = $this->label_art_detail($v,$GLOBALS['config']['view']['art_detail']);
                    $link = ffcms_url_art_detail($v, ['page' => $i]);

                    $this->buildHtml($link,'./', ffcms_tpl_fetch('art',$info['art_tpl'],'detail') );
                    if($i==1) {
                ...
                $GLOBALS['mid'] = 1;
                $GLOBALS['aid'] = ffcms_get_aid('vod','detail');

                $_REQUEST['id'] = $v['vod_id'];
                echo $v['vod_name'].'&nbsp;';;
                if(!empty($v['vod_play_from'])) {
                    $v['vod_play_list'] = ffcms_play_list($v['vod_play_from'], $v['vod_play_url'], $v['vod_play_server'], $v['vod_play_note'],'play');
                    $v['vod_play_total'] =  count($v['vod_play_list']);
                }
                if(!empty($v['vod_down_from'])) {
                    $v['vod_down_list'] = ffcms_play_list($v['vod_down_from'], $v['vod_down_url'], $v['vod_down_server'], $v['vod_down_note'],'down');
                    $v['vod_down_total'] =  count($v['vod_down_list']);
                }
                if(!empty($v['vod_plot_name'])) {
                    $v['vod_plot_list'] = ffcms_plot_list($v['vod_plot_name'], $v['vod_plot_detail']);
                ...
                if($GLOBALS['config']['view']['vod_detail'] == 2){
                    ...
                    $link = ffcms_url_vod_detail($v);
                    $this->buildHtml($link, './', ffcms_tpl_fetch('vod', $info['vod_tpl'], 'detail'));
                ...
                $flag = ['play','down'];
                foreach($flag as $f) {
                    $GLOBALS['aid'] = ffcms_get_aid('vod',$f);
                    ...
                    if ($GLOBALS['config']['view']['vod_'.$f] < 2) {
                        ...
                        if ($GLOBALS['config']['view']['vod_'.$f] == 2) {
                            ...
                            $link =  ($f=='play' ?ffcms_url_vod_play($v,['sid'=>1,'nid'=>1]) : ffcms_url_vod_down($v,['sid'=>1,'nid'=>1]) );
                            $this->buildHtml($link, './', ffcms_tpl_fetch('vod', $info['vod_tpl_'.$f], $f) );
                        ...
                        elseif ($GLOBALS['config']['view']['vod_'.$f] == 3) {
                            for ($i = 1; $i <= $v['vod_'.$f.'_total']; $i++) {
                                for ($j = 1; $j <= $v['vod_'.$f.'_list'][$i]['url_count']; $j++) {
                                    ...
                                    $link = ($f=='play' ? ffcms_url_vod_play($v, ['sid' => $i, 'nid' => $j]) : ffcms_url_vod_down($v, ['sid' => $i, 'nid' => $j]) );
                                    $link_sp = explode('?',$link);
                                    $this->buildHtml($link_sp[0], './', ffcms_tpl_fetch('vod', $info['vod_tpl_'.$f], $f) );
                                    if($i==1 && $j==1) {
                        ...
                        elseif ($GLOBALS['config']['view']['vod_'.$f] == 4) {
                            ...
                            for ($i = 1; $i <= $v['vod_'.$f.'_total']; $i++) {
                                ...
                                $info = $this->label_vod_play($f,$v,$GLOBALS['config']['view']['vod_'.$f]);
                                $link = ($f=='play' ? ffcms_url_vod_play($v, ['sid' => $i]) : ffcms_url_vod_down($v, ['sid' => $i]) );
                                $link_sp = explode('?',$link);
                                $this->buildHtml($link_sp[0], './', ffcms_tpl_fetch('vod', $info['vod_tpl_'.$f], $f) );
                                if($i==1) {
    ...
    public function label()
    {
        ...
        $GLOBALS['aid'] = ffcms_get_aid('label');
        if(empty($ids)){
            return $this->error('参数错误');
        }
        if(!is_array($ids)){
