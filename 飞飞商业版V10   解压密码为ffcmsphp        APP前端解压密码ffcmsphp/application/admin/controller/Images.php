...
class Images extends Base
{
    ...
    public function sync()
    {
        ...
        ffcms_echo('<span>共'.$total.'条数据需要处理，每页'.$param['limit'].'条，共'.$page_count.'页，正在处理第'.$param['page'].'页数据</span>');

        $list = Db::name($tab)->where($where)->page($page_count-1,$param['limit'])->select();
        $config = config('feifeicms.upload');
        ...
        foreach($list as $k=>$v){
            ...
            ffcms_echo($v[$col_id].'、'.$v[$col_name]);

            if($param['col'] == 2){
                $content = $v[$col_pic];
                $rule = ffcms_buildregx("<img[^>]*src\s*=\s*['".chr(34)."]?([\w/\-\:.]*)['".chr(34)."]?[^>]*>","is");
                ...
                foreach($matchfieldarr as $f=>$matchfieldstr)
                {
                    ...
                    if(in_array($config['mode'],['local'])){
                        $img_url = FFCMS_PATH . $img_url;
                        $link = $img_url;
                    }
                    else{
                        $link = str_replace('feifeicms:', $config['protocol'].':', $img_url);
                    }
                    if ($img_url == $img_old) {
