...
class Base extends All
{
    ...
    public function _cache_clear()
    {
        ...
        $content = 'FeifeiPlayerConfig.player_list='.json_encode($player) . ',FeifeiPlayerConfig.downer_list='.json_encode($downer) . ',FeifeiPlayerConfig.server_list='.json_encode($server) .';';
        $path = './static/js/playerconfig.js';
        if(!file_exists($path)){ $path .= '.bak'; }
        $fc = @file_get_contents( $path );
        $jsb = ffcms_get_body($fc,'//缓存开始','//缓存结束');
        $fc = str_replace($jsb,"\r\n".$content."\r\n",$fc);
