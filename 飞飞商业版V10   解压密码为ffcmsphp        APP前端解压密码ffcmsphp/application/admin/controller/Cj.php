...
class Cj extends Base
{
    ...
    public function col_content($param=[]) {
        ...
        ffcms_echo('正在采集内容，共【'.$total.'】条，分'.$total_page.'页，每页采集'.$limit.'条，当前'.$page.'页');
    ...
    public function content_into($param=[])
    {
        ...
        ffcms_echo('正在导入内容，共【'.$param['total'].'】条，分'.$total_page.'页，每页采集'.$param['limit'].'条，当前'.$param['page'].'页');
    ...
    public function export()
    {
        ...
        if(strpos($_SERVER['HTTP_USER_AGENT'], "MSIE")) {
            header("Content-Disposition: attachment; filename=ffcms_cj_" . urlencode($node['name']) . '.txt');
        }
        else{
            header("Content-Disposition: attachment; filename=ffcms_cj_" . $node['name'] . '.txt');
        }
