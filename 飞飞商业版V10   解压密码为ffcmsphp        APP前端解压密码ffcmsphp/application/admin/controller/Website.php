...
class Website extends Base
{
    ...
    public function batch()
    {
        ...
        if (!empty($param)) {
            ...
            if($param['ck_del'] == 1){
                ...
                ffcms_echo('批量删除完毕');
                ffcms_jump( url('website/batch') ,3);
                exit;
            }
            ...
            ffcms_echo( "<font color=red>共".$total."条数据需要处理，每页".$param['limit']."条，共".$page_count."页，正在处理第".$param['page']."页数据</font>");
    ...
    public function info()
    {
        if (Request()->isPost()) {
            $param = input('post.');
            $param['website_content'] = str_replace( $GLOBALS['config']['upload']['protocol'].':','feifeicms:',$param['website_content']);
