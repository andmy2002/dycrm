...
var FFCMS={
    ...
    'UserAgent' : function(){
        ...
        return {
            ...
            'ios': !!ua.match(/\(i[^;]+;( U;)? CPU.+Feifei OS X/), //ios终端
            'android': ua.indexOf('Android') > -1 || ua.indexOf('Linux') > -1, //android终端或者uc浏览器
            'iPhone': ua.indexOf('iPhone') > -1 || ua.indexOf('Feifei') > -1, //是否为iPhone或者QQHD浏览器
    ...
    'Home': function(o,u){
        try{
            ...
            if(window.netscape){
                ...
                catch(e){ FFCMS.Pop.Msg(150,40,'此操作被浏览器拒绝！请手动设置',1000); }
    ...
    'Fav': function(u,s){
        try{ window.external.addFavorite(u, s);}
            ...
            try{window.sidebar.addPanel(s, u, "");}catch (e){ FFCMS.Pop.Msg(150,40,'加入收藏出错，请使用键盘Ctrl+D进行添加',1000); }
        }
    },
    'Open': function(u,w,h){
    ...
    'History': {
        ...
        'Init':function(){
            ...
            html +='<dt><a target="_self" href="javascript:void(0)" onclick="FFCMS.History.Clear();">清空</a></dt>';
    ...
    'User':{
        ...
        'Init':function(){
            ...
            if(FFCMS.Cookie.Get('user_id') !=undefined && FFCMS.Cookie.Get('user_id')!=''){
				...
				var urlfavs = feifeicms.path + '/index.php/user/favs';
                ...
                if($('.ffcms_user').length >0) {
                    if ($('.ffcms_user').prop("outerHTML").substr(0, 2) == '<a') {
						$('.ffcms_user').attr('title', FFCMS.User.UserName);
						$('.ffcms_user').text('');
                        $('.ffcms_user').addClass('face_pic');
						$('.ffcms_user').append('<img class="face" src="' + FFCMS.User.Portrait + '" alt="会员头像">');
						var Ww = $(window).width();if (Ww < 820){$('.ffcms_user').attr('href', url);}
                    }
                    else {
                        //$('.ffcms_user').html('<a class="ffcms_text" href="'+ url +'">'+ name +'</a>');
                    }
					var VIP = '<i class="iconfont user_vip"></i>';
					if(FFCMS.User.GroupId <3){
						var GName = FFCMS.User.GroupName;
					}else{
						var GName = VIP + FFCMS.User.GroupName;
					}

                    var html = '<div class="ffcms_user_box dropbox user">';
                    html += '<div class="user_list_box"><div class="user_list"><a class="ffcms_user_n" href="javascript:;">'+FFCMS.User.UserName+'</a><a class="ffcms_user_g" href="javascript:;">' + GName + '</a><a href="' + url + '" target="_blank"><i class="iconfont"></i>个人中心</a><a href="' + urlfavs + '" target="_blank"><i class="iconfont"></i>我的收藏</a><a href="javascript:;" onclick="FFCMS.User.Logout();" target="_self"><i class="iconfont"></i>退出账号</a></div></div>'

                    $('.ffcms_user').after(html);
					$('.ffcms_user').removeClass('ffcms_user');
                }
